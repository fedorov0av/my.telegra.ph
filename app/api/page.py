...
from app.schemas.page import PageS, PageList
...
async def create_page(session: DBSessionDep, page: PageS, request: Request) -> PageS:
...
async def edit_page(session: DBSessionDep, page: PageS, request: Request) -> PageS:
    page_path = convert_text_for_url(page.page_path)
    page_url = str(request.base_url) + page_path
...
async def get_page_list(session: DBSessionDep, request: Request) -> PageList:
